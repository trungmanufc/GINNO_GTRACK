...
//		HAL_GPIO_WritePin(PWRKEY_CTRL_PORT, PWRKEY_CTRL_PIN, GPIO_PIN_SET);
		...
		Recv_Response(&UartEmulHandle, 400);
		Get_Paragraph(g_buff_temp, g_recv_buff, g_count_temp - 3, g_count_temp - 3);
		if(Compare_Str(g_buff_temp, (uint8_t*)"N", 1))
		{
				Log_Info((uint8_t*)"PWR DOWN!\n", 10);
...
/**
  ...
  */
uint32_t HTTP_Get(uint8_t timeOut)
{
		HAL_Delay(MAX_WAIT_TIME);
		uint8_t indexStart = 0, indexEnd = 0;
		sprintf((char*)g_buff_temp, "0AT+QHTTPGET=%d\r", timeOut);
		memset(g_recv_buff, '\0', MAX_SIZE_BUFF);
		Trans_Data(&UartEmulHandle, (uint8_t*)g_buff_temp, 16);
		Recv_Response(&UartEmulHandle, MAX_WAIT_TIME);
		while(strlen((char*)g_recv_buff) < 26) Recv_Response(&UartEmulHandle, MAX_WAIT_TIME);
		for(uint8_t i = 0; i < g_count_temp; i++)
		{
				if(g_recv_buff[i] == ':') 
				{
					indexStart = i;	//get index of ':' in response string
					break;
				}
		}
		indexStart +=2; //index of err in response string
		Get_Paragraph(g_buff_temp, g_recv_buff, indexStart, indexStart+4);
		if(Compare_Str(g_buff_temp, (uint8_t*)"0,200", 5))
		{
				Log_Info((uint8_t*)"Get success!\n", 13);
		}
		else 
		{
				Log_Info((uint8_t*)"Get fail!\n", 10);
				return 0;
		}
		indexStart += 6; //index start of <len data>
		for(uint8_t i = indexStart; i < g_count_temp; i++)
		{
				if(g_recv_buff[i] == '\r') 
				{
					indexEnd = i - 1 ;	//get index of end <len data>
					break;
				}
		}
		memset(g_buff_temp, '\0', MAX_SIZE_BUFF);
		Get_Paragraph(g_buff_temp, g_recv_buff, indexStart, indexEnd);
		Log_Info(g_buff_temp, indexEnd - indexStart +1);
		return atoi((char*)g_buff_temp);
}

/**
  * @brief  Get a range data request
	* @param  timeOut: timeout for the HTTP get response
	* @param  startPosition: start position of data string
	* @param  range: length of data client wanna get
  * @retval OK or ERR
  */
response_t HTTP_Get_Range(uint8_t* timeOut, 
													uint8_t* startPosition, uint8_t* range)
{
		HAL_Delay(MAX_WAIT_TIME);
		uint8_t indexStart = 0, indexEnd = 0;
		uint8_t lenBuffTrans = 18 + strlen((char*)timeOut) + strlen((char*)startPosition) + strlen((char*)range);
		sprintf((char*)g_buff_temp, "0AT+QHTTPGETEX=%s,%s,%s\r", timeOut, startPosition, range);
		Trans_Data(&UartEmulHandle, (uint8_t*)g_buff_temp, lenBuffTrans);
		Recv_Response(&UartEmulHandle, MAX_WAIT_TIME);
		
		for(uint8_t i = g_count_temp - 1; i > 0; i--)
		{
				if(g_recv_buff[i] == ' ')
				{
						indexStart = i;
		...
		indexStart +=1; //index of <err> in response string
		if(g_recv_buff[indexStart] == '0')
		{
				Log_Info((uint8_t*)"Get range success!\n", 19);
		}
		else return RESPONSE_ERR;
		for(uint8_t i = indexStart; i < g_count_temp; i++)
		{
		...
		indexStart = indexEnd - strlen((char*)range) + 1; //index start of <range>
		memset(g_buff_temp, '\0', MAX_SIZE_BUFF);
		Get_Paragraph(g_buff_temp, g_recv_buff, indexStart, indexEnd);
		if(Compare_Str(g_buff_temp, range, strlen((char*)range)))
		{
				Log_Info((uint8_t*)"Get range done!\n", 16);
				return RESPONSE_OK;
		}
		return RESPONSE_ERR;
}
...
/**
  * @brief  Search first of small string  in big string
	* @param  strBig: string big
	* @param  strSmall: string small
  * @retval Index of Small String (OK) or strlen of Big String (ERR)
  */
uint16_t Search_First_Str(uint8_t* strBig,
													uint8_t* strSmall)
{
    uint16_t idBig = 0, idSmall = 0;
    uint8_t lenSmall = strlen((char*)strSmall);

    while(strBig[idBig] != '\0')
    {
        if(strBig[idBig] == strSmall[idSmall])
        {
            while((strBig[idBig] == strSmall[idSmall]) && (strBig[idBig] != '\0'))
            {
                idBig++;
                idSmall++;
            }
            if(idSmall == lenSmall) //if len of paragraph = len of Small Str
                return (idBig - lenSmall);  //return index of paragraph
        }
        else
        {
            idBig++;
            idSmall = 0; //reset index Small
        }
    }
    return strlen((char*)strBig);
}
...
/**
  ...
  */
uint16_t HTTP_Read(uint8_t timeOut,
									 uint16_t lenData)
{
		HAL_Delay(MAX_WAIT_TIME);
	  memset(g_recv_buff, '\0', MAX_SIZE_BUFF);

		uint16_t indexStart = 0, indexEnd = 0;
		sprintf((char*)g_buff_temp, "0AT+QHTTPREAD=%d\r", timeOut);
		memset(g_recv_buff, '\0', MAX_SIZE_BUFF);
		Trans_Data(&UartEmulHandle, (uint8_t*)g_buff_temp, 17);
		Recv_Response(&UartEmulHandle, WAIT_READ);
//		while(strlen((char*)g_recv_buff) < 26) Recv_Response(&UartEmulHandle, MAX_WAIT_TIME);
		for(uint16_t i = g_count_temp - 1; i > 0; i--)
		{
			if(g_recv_buff[i] == ':') 
				{
					indexStart = i;	//get index of ' ' in response string
		...
		if(g_recv_buff[indexStart] == '0')
		{
				Log_Info((uint8_t*)"Read success!\n", 14);
		}
		else 
		{
				Log_Info((uint8_t*)"Read fail!\n", 11);
				return 0;
		}
		indexStart = 0;
		indexStart = Search_First_Str(g_recv_buff, (uint8_t*)"\r\n\r\n") + 4;	//start get data 
		indexEnd = indexStart + (lenData - 1);

		Log_Info((uint8_t*)"-",1);
		Log_Info(&g_recv_buff[indexStart], 1);
		Log_Info(&g_recv_buff[indexStart+1], 1);
		Log_Info((uint8_t*)"-",1);
		Log_Info((uint8_t*)"+",1);
		Log_Info(&g_recv_buff[indexEnd-1], 1);
		Log_Info(&g_recv_buff[indexEnd], 1);
		Log_Info((uint8_t*)"+",1);
		memset(g_buff_temp, '\0', MAX_SIZE_BUFF);
		Get_Paragraph(g_buff_temp, g_recv_buff, indexStart, indexEnd);
		Log_Info((uint8_t*)"/",1);
		Log_Info(&g_buff_temp[0], 1);
		Log_Info(&g_buff_temp[lenData-1], 1);
		Log_Info((uint8_t*)"/",1);
